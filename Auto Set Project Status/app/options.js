    ...
    if (bodyitems && projectOptions) {
        ...
        if (projectOptions.length > 1) {
            for (var i = 0; i < projectOptions.length; i++) {
                ...
                att.value = projectOptions[i].value;
                node.setAttributeNode(att);
                var t = document.createTextNode(projectOptions[i].label);
                node.appendChild(t);
            ...
            chrome.tabs.executeScript(null, { file: "jquery_1.8.3.min.js" }, function () {
                chrome.tabs.executeScript({
                    code: '$("#cbProjectsList").val("' + projectOptions[i].value + '");$("#pbGo").removeAttr("disabled");$("#pbGo").trigger("click");'
                });
                window.close();
            });
        }
